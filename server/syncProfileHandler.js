 ...
 */
const { bindSocketHandler, logSocketError } = require('./socketLog');
const {
  persistCloudProfile,
  hasCloudSavePayload,
...
function registerSyncProfileHandler(ctx) {
  ...
  bindSocketHandler(socket, 'syncProfile', async ({ socket: sock, payload, ack, profileId }) => {
    const roomCodeHint = payload.roomCode;
    const storeCfg = getCloudStoreConfig();

    console.log('[syncProfile] payload summary', {
      socketId: sock.id,
      profileId,
      roomCode: roomCodeHint || null,
      hasCloudDocument: !!payload.cloudDocument,
      cloud: redactCloudPayloadForLog(payload.cloudDocument),
      storeMode: storeCfg.mode,
      dynamoTable: storeCfg.tableName,
    });

    const { code, room } = findRoomForSocket(sock.id, roomCodeHint);
    if (!code || !room) {
      ack({
        ok: false,
        error: 'Not in a room',
        details: 'Join or create a room before syncing profile',
        errorName: 'NOT_IN_ROOM',
      });
      return;
    }
    ...
    if (!slot) {
      ack({
        ok: false,
        error: 'Not in room',
        details: 'Socket is not assigned to p1 or p2',
        errorName: 'NO_SLOT',
      });
      return;
    }
    ...
    if (hasCloudSavePayload(payload)) {
      const playerKey = String(payload.playerKey || payload.cloudDocument.playerKey || '')
        .replace(/\D/g, '')
        .slice(0, 4);
      console.log('[syncProfile] cloud persist starting', {
        profileId: p.profile.profileId,
        mode: storeCfg.mode,
        table: storeCfg.tableName,
        apiBase: storeCfg.saveApiBase ? `${storeCfg.saveApiBase.slice(0, 48)}…` : null,
      });
      cloud = await persistCloudProfile(payload.cloudDocument, playerKey);
      if (!cloud.ok && !cloud.skipped) {
        ack({
          ok: false,
          error: cloud.error || 'Cloud save failed',
          details: cloud.details || '',
          errorName: cloud.errorName || 'CLOUD_SAVE_FAILED',
    ...
    console.log('[syncProfile] success', {
      ...
      profileId: p.profile.profileId,
      cloudOk: cloud.ok,
      cloudSkipped: cloud.skipped,
    });
    ack({ ok: true, roomCode: code, playerSlot: slot, cloud });
  });
}
